...
namespace {
    constexpr size_t kMaxMapSamples = 50;
    constexpr size_t kMaxStringSamples = 30;
    constexpr size_t kMaxEntrySamples = 200;
    constexpr int kFieldScanBytes = 64;
}
...
void DynamicFinder::ScanMapOffsets() {
    ...
    const auto& maps = GetMapStructures();
    if (maps.empty()) {
    ...
    for (auto mapAddr : maps) {
        auto fields = ReadFields(mapAddr, kFieldScanBytes);
        ...
        for (auto& [offset, value] : fields) {
            ...
            if (IsValidPointer(value) && IsHeapMemory(value)) {
                ...
                const bool hasLeftChild = IsHeapMemory(nodeLeft);
                const bool hasRightChild = IsHeapMemory(nodeRight);
...
void DynamicFinder::ScanEntryOffsets() {
    ...
    for (size_t i = 0; i < entries.size(); i++) {
        auto fields = ReadFields(entries[i], kFieldScanBytes);
...
const std::vector<uintptr_t>& DynamicFinder::GetMapStructures() const {
    if (!cachedMaps.has_value()) {
        cachedMaps = FindMapStructures();
    }
    return *cachedMaps;
}

std::vector<uintptr_t> DynamicFinder::FindMapStructures() const {
    ...
    scanner.ScanMemory([&](uintptr_t addr, const std::vector<uint8_t>& data) -> bool {
        for (size_t i = 0; i + 32 <= data.size(); i += 8) {
            ...
            if (ptrCount >= 2 && ptrCount <= 4 && intCount >= 1) {
                ...
                if (maps.size() >= kMaxMapSamples) return true; // Đủ sample
            }
        }
        return false;
    });
...
std::vector<uintptr_t> DynamicFinder::FindEntryStructures(const std::vector<uintptr_t>& maps) const {
    ...
    for (auto mapAddr : maps) {
        ...
        for (auto& [offset, value] : fields) {
            ...
            if (IsHeapMemory(left) && IsHeapMemory(right)) {
                // Traverse tree with a hard sample cap so corrupt candidates cannot
                // cause unbounded recursion or very long scans.
                std::function<void(uintptr_t)> traverse = [&](uintptr_t node) {
                    if (!node || visited.count(node) || entries.size() >= kMaxEntrySamples) return;
                    if (!IsHeapMemory(node)) return;
                    visited.insert(node);
...
std::vector<uintptr_t> DynamicFinder::FindStringStructures() const {
    ...
    scanner.ScanMemory([&](uintptr_t addr, const std::vector<uint8_t>& data) -> bool {
        for (size_t i = 0; i + 24 <= data.size(); i += 8) {
            ...
            const auto dataPtr = Utils::ReadUnaligned<uint64_t>(data, i).value_or(0);
            const auto size = Utils::ReadUnaligned<uint64_t>(data, i + 8).value_or(0);
            const auto cap = Utils::ReadUnaligned<uint64_t>(data, i + 16).value_or(0);
            ...
            if (IsHeapMemory(dataPtr) && size > 0 && size < 10000 && cap >= size) {
                ...
                if (!strData.empty()) {
                    ...
                    if (isPrintable) {
                        ...
                        if (strings.size() >= kMaxStringSamples) return true;
                    }
                }
            }
        }
...
std::map<int, uint64_t> DynamicFinder::ReadFields(uintptr_t addr, int maxBytes) const {
    ...
    for (size_t off = 0; off + 8 <= data.size(); off += 8) {
        uint64_t val = Utils::ReadUnaligned<uint64_t>(data, off).value_or(0);
        if (val != 0) {
            fields[off] = val;
        }
    }
...
bool DynamicFinder::IsLikelyStringData(uint64_t value) const {
    ...
    // Check inline string without type-punning through a pointer cast.
    char chars[sizeof(value)]{};
    std::memcpy(chars, &value, sizeof(value));
    int len = 0;
    for (int i = 0; i < static_cast<int>(sizeof(chars)) && chars[i] != 0; i++) {
...
bool DynamicFinder::ExportToFile(const std::string& filename) const {
...
bool DynamicFinder::ExportHeader(const std::string& filename) const {
    std::ofstream file(filename);
    if (!file.is_open()) {
        Utils::LogError("Cannot write to " + filename);
        return false;
    ...
    for (const auto& r : results) {
        if (r.found) {
            file << "inline constexpr uintptr_t " << r.name << " = 0x" << std::hex << r.offset << std::dec << ";\n";
        } else {
            file << "// inline constexpr uintptr_t " << r.name << " = NOT_FOUND;\n";
        }
    }

    file.close();
    Utils::LogSuccess("Exported header to: " + filename);
    return true;
}